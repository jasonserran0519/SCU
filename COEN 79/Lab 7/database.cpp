...
namespace coen79_lab7 {
    ...
    bool database::insert_item(const std::string &company, const std::string &product_name, const float &price) {
        ...
        size_type index = search_company(company);                              //checks if the company exists
        if(index == COMPANY_NOT_FOUND)
            return false;
        else 
            company_array[index].insert(product_name, price);                               // Inserts into the last item of the company
            return true;
    }
    ...
    bool database::erase_item(const std::string& cName, const std::string& pName) {
        ...
        if(index == COMPANY_NOT_FOUND)                                                      // Check if the company exists
            return false;                                                      // Check if the company exists
            return false;
    ...
    database::size_type database::search_company(const std::string& company) {
        ...
        for(size_type i = 0; i < used_slots; i++)                                           // Iterate through the companies
            if(company_array[i].get_name() == company)                                      // If a match, return the index
                return i;                                                                   
